...
GFX_TEST(gfx_brutal_logo_fill)
{
    gfx_push(test_gfx());
    gfx_origin(test_gfx(), m_vec2f(64, 64));
    gfx_fill_style(test_gfx(), gfx_paint_fill(GFX_WHITE));
    gfx_fill_svg(test_gfx(), str$(_logo), GFX_FILL_EVENODD);

    gfx_pop(test_gfx());
}

GFX_TEST(gfx_brutal_logo_stroke)
{
    gfx_push(test_gfx());
    gfx_origin(test_gfx(), m_vec2f(64, 64));
    gfx_fill_style(test_gfx(), gfx_paint_fill(GFX_WHITE));
    gfx_stroke_svg(test_gfx(), str$(_logo));

    gfx_pop(test_gfx());
}

GFX_TEST_COUNT(gfx_brutal_logo_rotated, 360)
{
    MTrans2 trans = m_trans2_rotate(m_deg2rad(m_deg(test_iteration())).v);
    gfx_push(test_gfx());
    gfx_origin(test_gfx(), m_vec2f(500, 300));
    gfx_trans(test_gfx(), trans);
