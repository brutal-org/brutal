...
{
    return cexpr_cast(cexpr_ident(bid_cgen_pack_name(name, alloc)), ctype_ident_ptr(str$("BalPackFn"), alloc), alloc);
}
...
void bid_cgen_pack_body(CStmt *block, BidType type, CExpr path, Alloc *alloc)
{
    ...
    {
        CExpr expr = cexpr_call(alloc, cexpr_ident(bid_cgen_pack_name(type.primitive_.mangled, alloc)));
    ...
    case BID_TYPE_ENUM:
    {
        CExpr expr = cexpr_call(alloc, cexpr_ident(bid_cgen_pack_name(str$("Enum"), alloc)));
        cexpr_member(&expr, cexpr_ident(str$("self")));
    ...
    case BID_TYPE_STRUCT:
    {
        ...
        {
            bid_cgen_pack_body(block, member.type, cexpr_ref(cexpr_ptr_access(path, cexpr_ident(member.name), alloc), alloc), alloc);
        }
        break;
    }
    ...
    case BID_TYPE_VEC:
    {
        ...
        cexpr_member(&expr, cexpr_ident(str$("self")));
        cexpr_member(&expr, path);
        cexpr_member(&expr, cexpr_ident(bid_cgen_pack_name(subtype.primitive_.mangled, alloc)));
        cstmt_block_add(block, cstmt_expr(expr));
        break;
    }
...
{
    Str name = bid_cgen_pack_name(alias.mangled, alloc);
    CType type = bid_cgen_pack_type(alias, alloc);
    CStmt body = cstmt_block(alloc);
    bid_cgen_pack_body(&body, alias.type, cexpr_ident(str$("data")), alloc);
