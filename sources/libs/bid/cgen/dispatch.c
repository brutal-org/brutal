...
void bid_cgen_dispatch_case(CStmt *block, BidMethod method, BidIface const iface, Alloc *alloc)
{
    ...
    cexpr_member(&call_handler, bid_cgen_dispatch_handler(method, alloc));
    cexpr_member(&call_handler, cexpr_ident(str$("ev")));
    cexpr_member(&call_handler, cexpr_ident(str$("req")));
    cexpr_member(&call_handler, bid_cgen_binding(method, iface, alloc));

    CExpr nullptr_ident = cexpr_ident(str$("nullptr"));

    if (method.request.type != BID_TYPE_NIL)
    {
        cstmt_block_add(block, cstmt_decl(cdecl_var(str$("req_buf"), bid_cgen_decl_primitive(method.request), cexpr_empty()), alloc));
    ...
    {
        cstmt_block_add(block, cstmt_decl(cdecl_var(str$("resp_buf"), bid_cgen_decl_primitive(method.response), cexpr_empty()), alloc));
...
CStmt bid_cgen_dispatch_body(BidIface const iface, Alloc *alloc)
{
    ...
    vec_foreach_v(method, &iface.methods)
    {
        ...
        bid_cgen_dispatch_case(&case_body, method, iface, alloc);
...
CDecl bid_cgen_dispatch_func(BidIface const iface, Alloc *alloc)
{
    ...
    CType type = bid_cgen_dispatch_type(alloc);
    CStmt body = bid_cgen_dispatch_body(iface, alloc);
