...
{
    while (!skip_keyword(scan, "*/") && !scan_ended(scan))
    {
        scan_next(scan);
...
static UdExpr parse_rune(Scan *scan)
{
    ...
    if (scan_curr(scan) == 'x' && isxdigit(scan_peek(scan, 1)))
    {
        ...
        while (isxdigit(scan_curr(scan)) && !scan_ended(scan))
        {
            rune *= 16;
...
{
    if (scan_skip(scan, ')') || scan_ended(scan))
    {
        return ud_nil();
    }
...
{
    if (scan_curr(scan) == ';' || scan_ended(scan))
    {
        return ud_nil();
    }
