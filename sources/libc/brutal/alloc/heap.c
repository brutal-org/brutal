...
#define MAJOR_BLOCK_HEADER_SIZE (ALIGN_UP(sizeof(struct alloc_major), 16))
...
#define MINOR_BLOCK_HEADER_SIZE (ALIGN_UP(sizeof(struct alloc_minor), 16))
...
{
    if (min->magic == ALLOC_GLOBAL_MAGIC)
    {
        return true;
    }

    // Check for overrun errors. For all bytes of ALLOC_GLOBAL_MAGIC
    if (((min->magic & 0xFFFFFF) == (ALLOC_GLOBAL_MAGIC & 0xFFFFFF)) ||
...
void *alloc_heap_acquire(struct alloc_heap *alloc, size_t req_size)
{
    ...
    while (maj != nullptr)
    {
        ...
        if (maj->first == nullptr)
        {
            ...
            maj->first->magic = ALLOC_GLOBAL_MAGIC;
        ...
        while (min != nullptr)
        {
            ...
            if (min->next != nullptr)
            {
                ...
                if (diff >= (size + MINOR_BLOCK_HEADER_SIZE))
                {
                    ...
                    new_min->magic = ALLOC_GLOBAL_MAGIC;
